      ...
      <ScrollView className='flex-1' contentContainerClassName='pb-32'>
        ...
        <View className='mt-6 flex-row px-6'>
          <View className='mr-3 flex-1 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm'>
            <View className='mb-2 flex-row items-center'>
              ...
              <Text className='font-inter-medium ml-2 text-xs text-slate-600'>30-day average</Text>
            </View>
          ...
          <View className='ml-3 flex-1 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm'>
            <View className='mb-2 flex-row items-center'>
              <Ionicons name='trending-up-outline' size={16} color='#64748b' />
              <Text className='font-inter-medium ml-2 text-xs text-slate-600'>7-day trend</Text>
            </View>
        ...
        {thisWeek.length > 0 && (
          <View className='mt-8 px-6'>
            <Text className='font-inter-semibold mb-4 text-base text-slate-900'>This week</Text>
            {thisWeek.map((entry) => {
              const { dateStr, time } = formatDateTime(entry);
              return (
                <View
                  key={entry.id}
                  className='mb-3 flex-row items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-sm'>
                  <View className='flex-1 flex-row items-center'>
                    ...
                    <View className='flex-1'>
                      ...
                      <View className='flex-row items-center'>
                        <Text className='font-inter-regular text-sm text-slate-500'>{dateStr}</Text>
                        <Text className='font-inter-regular ml-2 text-xs text-slate-400'>
                          • {time}
                        </Text>
                      </View>
        ...
        {lastWeek.length > 0 && (
          <View className='mt-6 px-6'>
            <Text className='font-inter-semibold mb-4 text-base text-slate-900'>Last week</Text>
            {lastWeek.map((entry) => {
              const { dateStr, time } = formatDateTime(entry);
              return (
                <View
        ...
        {entries.length === 0 && (
          <View className='mt-20 items-center px-8'>
            ...
            <Text className='font-inter-semibold mt-4 text-center text-xl text-slate-900'>
              No check-ins yet
            </Text>
            <Text className='font-inter-regular mt-2 text-center text-base text-slate-600'>
