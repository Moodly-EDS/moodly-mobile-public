        ...
        <View className='px-6 pb-32'>
          {statsLoading ? (
            <View className='items-center justify-center py-20'>
              ...
              <Text className='font-inter-medium mt-4 text-sm text-slate-500'>
            ...
            <>
              ...
              <View className='mt-8 mb-6'>
                ...
                <View className='mt-1 flex-row items-center'>
                  ...
                  <Text className='font-inter-medium ml-1 text-sm text-slate-500'>
                    Engineering Team
                  </Text>
                </View>
                <Text className='font-inter-regular mt-1 text-xs text-slate-400'>
                  30-day anonymous, aggregated team trends
                </Text>
              </View>

              {/* 30-day Average Card */}
              <View className='mb-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm'>
                <Text className='font-inter-medium mb-4 text-sm text-slate-600'>
                  30-day average
                </Text>
                <View className='items-center'>
                  ...
                  <Text className='mt-4 text-4xl'>
                    {teamStats.average >= 4.5
                      ? '😄'
                      : teamStats.average >= 3.5
                        ? '🙂'
                        : teamStats.average >= 2.5
                          ? '😐'
                          : teamStats.average >= 1.5
                            ? '😟'
                            : '😞'}
                  </Text>
                </View>
              ...
              <View className='mb-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm'>
                ...
                <View
                  className={`mb-4 flex-row items-center rounded-2xl p-4 ${
                    teamStats.last7Days.trend === 'Rising'
                      ? 'bg-green-50'
                      : teamStats.last7Days.trend === 'Falling'
                        ? 'bg-red-50'
                        : 'bg-slate-50'
                  }`}>
                  <View
                  ...
                  <View className='flex-1'>
                    ...
                    <Text
                      ...
                      {teamStats.last7Days.trendDiff > 0 ? '+' : ''}
                      {teamStats.last7Days.trendDiff.toFixed(2)} vs prev week
                    </Text>
                  </View>
                </View>
              ...
              <View className='mb-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm'>
                ...
                <Text className='font-inter-regular mb-4 text-xs text-slate-500'>
                ...
                <View className='mb-6 space-y-3'>
                  {[
                    ...
                    const count =
                      teamStats.last7Days.moodDistribution[
                        item.mood as keyof typeof teamStats.last7Days.moodDistribution
                      ];
                    const percentage =
                      teamStats.last7Days.totalCheckins > 0
                        ? (count / teamStats.last7Days.totalCheckins) * 100
                        : 0;

                    return (
                      <View key={index} className='flex-row items-center'>
                        <Text className='mr-3 text-2xl' style={{ width: 30 }}>
                          {item.emoji}
                        </Text>
                        <View className='flex-1'>
                          <View
              ...
              <View className='rounded-3xl bg-slate-50 p-6'>
                <View className='flex-row'>
                  ...
                  <View className='ml-3 flex-1'>
                    <Text className='font-inter-semibold mb-2 text-sm text-slate-900'>
                      About this data:
                    </Text>
                    <Text className='font-inter-regular text-xs leading-5 text-slate-600'>
                      All check-ins are anonymous and aggregated at the team level. Trends are only
                      shown when there are sufficient responses to preserve individual privacy.
                    </Text>
                  </View>
                </View>
              </View>
