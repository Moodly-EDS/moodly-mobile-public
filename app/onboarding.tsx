    ...
    <View className='flex-1 bg-white'>
      <View className='mt-18 items-center'>
        <View className='flex-row items-center'>
          ...
          <Text className='font-inter-semibold ml-2 text-2xl text-slate-900'>Moodly</Text>
        </View>
      </View>
      ...
      <ScrollView
        ...
        {slides.map((slide, index) => (
          <View key={index} style={{ width }} className='flex-1 items-center justify-center px-8'>
            ...
            <Text className='font-inter-bold mb-4 text-center text-3xl text-slate-900'>
              {slide.title}
            </Text>

            <Text className='font-inter-regular text-center text-base leading-6 text-slate-600'>
              {slide.description}
            </Text>
          </View>
        ))}
        ...
        {slides.map((_, index) => (
          <View
            ...
            className={`mx-1 h-2 rounded-full ${
              index === currentPage ? 'w-8 bg-blue-600' : 'w-2 bg-slate-300'
            }`}
          />
        ))}
      </View>
      ...
      <View className='mb-12 flex-row items-center justify-between px-8'>
        <TouchableOpacity onPress={handleSkip} className='px-6 py-4'>
          <Text className='font-inter-medium text-base text-slate-600'>Skip</Text>
        </TouchableOpacity>

        <TouchableOpacity
          onPress={handleNext}
          className='flex-row items-center rounded-full bg-blue-600 px-8 py-4'>
          <Text className='font-inter-semibold mr-2 text-base text-white'>
