...
export const ToastProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <ToastContext.Provider value={{ showToast, showSuccess, showError, showInfo, showWarning }}>
      {children}
      <View className='absolute top-16 right-0 left-0 z-50 px-4'>
        {toasts.map((toast) => (
          <ToastItem
            key={toast.id}
            toast={toast}
...
const ToastItem: React.FC<{ toast: Toast; onDismiss: () => void }> = ({ toast, onDismiss }) => {
  ...
  return (
    <Animated.View
      ...
      <Pressable
        onPress={onDismiss}
        className={`${getBackgroundColor()} flex-row items-center rounded-2xl px-4 py-3 shadow-lg`}>
        <Text className='mr-2 text-lg font-bold text-white'>{getIcon()}</Text>
